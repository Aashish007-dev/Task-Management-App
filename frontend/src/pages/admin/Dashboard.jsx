          ...
          {dashboardData && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ">
              <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-blue-500">
                ...
                <p className='text-3xl font-bold text-gray-800 mt-2'>{dashboardData?.charts?.taskDistribution?.All || 0}</p>
              </div>

              <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-yellow-500">
                <h3 className='text-gray-500 text-sm font-medium'>Pending Tasks</h3>
                <p className='text-3xl font-bold text-gray-800 mt-2'>{dashboardData?.charts?.taskDistribution?.Pending || 0}</p>
              </div>

              <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-red-500">
                <h3 className='text-gray-500 text-sm font-medium'>In Progress Tasks</h3>
                <p className='text-3xl font-bold text-gray-800 mt-2'>{dashboardData?.charts?.taskDistribution?.InProgress || 0}</p>
              </div>

              <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-green-500">
                <h3 className='text-gray-500 text-sm font-medium'>Completed Tasks</h3>
                <p className='text-3xl font-bold text-gray-800 mt-2'>{dashboardData?.charts?.taskDistribution?.Completed || 0}</p>
              </div>
            </div>
          )}
